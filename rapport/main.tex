...
\begin{itemize}[itemsep=0pt]
    \item \py{kuramoto(t, show=True)}
    \item \py{all_kuramoto()}
    \item \py{anim_kuramoto()}
    \item \py{dens_kuramoto()}
    \item \py{dens_kuramoto_coord(t, show)}
    \item \py{anim_dens_kuramoto_coord()}
    \item \py{orders()}
    \item \py{shannon(t, pol=False)}
    \item \py{dens_shannon()}
    \item \py{dens_shannon_coord(t, show=True)}
    \item \py{anim_dens_shannon_coord(t)}
    \item \py{connectivity(K, kmin)}
    \item \py{graph_connectivity(kmin=0)}
\end{itemize}
\noindent

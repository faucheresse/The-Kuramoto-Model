...
\item $\dot{\theta^i}(t)$ represent the varation of the phase of the i-th oscillator over time.
...
\begin{itemize}[itemsep=0pt]
    \item \py{kuramoto(t, show=True)} : plot $i\longmapsto\{\theta^i(t)\}_{i=0,...,N-1}$ at a time $t$. The argument \py{show} is used by the animation function, it allows to not display the graph but save it in the animation directory (if \py{show==False}).
    \item \py{all_kuramoto()} : plot the N curves $t\longmapsto\theta^i(t)$
    \item \py{anim_kuramoto()} : using \py{kuramoto(t, show=False)} it take all the \py{"kuramoto*.png"} in the animation directory and create a gif with them.
    \item \py{dens_kuramoto()} : plot $(i,t)\longmapsto \theta^i (t)$ using the \py{numpy.contourf()} method. 
    \item \py{dens_kuramoto_coord(t, show=True)} : plot $(r,c)\longmapsto \theta^{i_{r,d}} (t)$ at time $t$ using the \py{numpy.contourf()} method.
    ...
    \item \py{shannon(t)} : plot $i \longmapsto S_i^{q, n}(t)$ at a time $t$.
    \item \py{dens_shannon()} : plot $(i,t) \longmapsto S_i^{q, n}(t)$ using the \py{numpy.contourf()} method.
    \item \py{dens_shannon_coord(t, show=True)} : plot $(r,c) \longmapsto S_i^{q, n}(t)$ at time $t$ using the \py{numpy.contourf()} method.
...
This class is the main class of the program. Here is a table of the class:
\begin{table}[htbp]
    \begin{center}
        \begin{tabular}{p{0.4\linewidth} p{0.4\linewidth}} \toprule
            ...
            This class contains 9 attributes, see \autoref{subs:1.1}:
            \begin{itemize}[leftmargin=15pt, itemsep=0pt, topsep=0pt]
                \item\py{N,type:int}
                \item\py{omega,type:ndarray,shape:(N,N)}
                \item\py{eta,type:ndarray,shape:(N,T)}
            ...
            \begin{itemize}[leftmargin=15pt, itemsep=0pt, topsep=0pt]
                \item \py{__call__(theta, t)} : return \py{d_theta} using the kuramoto model, \autoref{subs:2.1}.
                ...
                \item \py{shannon_entropy(theta, i)} : return the computation of the Shannon entropy for an oscillator.
